 ...
 *      Payslips come from the Upload Portal's Payslips tab (private storage,
 *      through the payslips edge function), not from Drive, so there is no
 *      folder to set up.
 *      Only the public anon key lives in this file: it can't read any perf_ table
 *      or the payslips bucket, only call the token-checked functions, same as the
 *      page itself.
 *   3. Run draftMonthlyEmails() once to approve permissions (Gmail, fetch).
 ...
 *
 * EACH MONTH
 *   - Accounts upload the payslip PDFs in the Upload Portal → Payslips tab.
 ...
 *   - Payslips late? Upload them and run draftMonthlyEmails() again: a draft that was
...
// Their payslip PDF for the month, from the Upload Portal's Payslips tab. The
// payslips edge function checks the person's own token and returns a signed
// link that lasts ten minutes; null when nothing has been uploaded yet.
function fetchPayslip_(token, month, name) {
  const r = UrlFetchApp.fetch(`${PERF_SUPA_URL}/functions/v1/payslips`, {
    method: 'post', contentType: 'application/json', muteHttpExceptions: true,
    headers: { apikey: PERF_ANON_KEY, Authorization: `Bearer ${PERF_ANON_KEY}` },
    payload: JSON.stringify({ action: 'mine', token, month }),
  });
  if (r.getResponseCode() !== 200) throw new Error(`payslips ${r.getResponseCode()}: ${r.getContentText()}`);
...
function perfEmailHtml_(d, token, month, hasPayslip) {
  ...
  const six = [
    ...
    // Reviews that name them, matched through staff_name_variants (typos and
    // nicknames like Andy or Edz count), same number as their page.
    ['Google reviews', 'google_reviews', 'num'],
  ].map(([l, k, f]) => {
  ...
  // Their best review of the month (highest stars, then newest), as a short quote.
  const best = (n.review_list || []).filter(r => r.stars >= 4 && r.comment)
    .sort((a, c) => c.stars - a.stars || String(c.date).localeCompare(String(a.date)))[0];
  const quote = best ? (best.comment.length > 220 ? best.comment.slice(0, 217).replace(/\s+\S*$/, '') + '…' : best.comment) : '';
...
function draftMonthlyEmails(month) {
  ...
  GmailApp.createDraft(me, `Performance drafts ready · ${month}`, 'See HTML version.', {
    htmlBody: `<div style="font-family:Arial,sans-serif">
      ...
      <p>When they look right, run <b>sendMonthlyDrafts()</b>. Late payslips: upload them in the Upload Portal → Payslips tab and run draftMonthlyEmails() again.</p></div>`,
  });
